...
IMPORT MODE LINKS & BOLD: every URL/hyperlink in the file must survive. For EVERY project and experience entry that has any link on its title line (a hyperlinked project name, a "Live Demo"/"GitHub"/"Link" text, or a bare URL next to the title), you MUST fill that entry's "linkLabel" and "linkUrl" fields and remove the raw URL from "title"/"subtitle". Never leave a project link only inside the title text.
IMPORT MODE BOLD: reproduce the ** ** bold markers EXACTLY as they appear in the input — same words, no more, no less. Do not bold extra keywords, do not un-bold anything that was bold.
IMPORT MODE SECTIONS: keep every item in the SAME section it was under in the file. Degrees, diplomas, schools, colleges, universities, 10th/12th, B.Tech/M.Tech/BCA/MCA/B.Sc and CGPA lines belong in "education" and must NEVER be moved into "certifications". Only items listed under a Certifications/Courses/Training heading go into "certifications". If a job/project/certification title or heading has a hyperlink attached to it, put the readable label in "linkLabel" and the URL in "linkUrl" for that entry. Links that belong to personal details (email, phone, LinkedIn, GitHub, portfolio) go into "contact" as [Label](url). Links inside a bullet stay inline in that bullet as [Label](url). Any word that was **bold** in the file must remain wrapped in ** ** in the output.` : ""}
